...
abstract class GnuBoardCrawler(
    ...
    override fun links(document: Document) =
        document.select(".notice_list > table > tbody > tr a[href*=wr_id]")

    override fun articleNo(link: Element) = parameter(link.attr("href"), "wr_id")

    override fun detailUrl(link: Element, articleNo: String) = "$baseUrl&wr_id=$articleNo"
