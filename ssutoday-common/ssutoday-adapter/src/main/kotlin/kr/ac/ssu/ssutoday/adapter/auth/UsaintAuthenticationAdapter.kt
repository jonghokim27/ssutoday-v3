...
class UsaintAuthenticationAdapter(
    ...
    override fun authenticate(
        ...
        if (!sso.body.orEmpty().contains("/irj/portal")) throw BusinessException(StatusCode.SSU4010)
        ...
        val html =
            client.exchange(portalUrl, HttpMethod.GET, HttpEntity<Unit>(portalHeaders), String::class.java).body
                ?: throw BusinessException(StatusCode.SSU5000)
        val document = Jsoup.parse(html)
        val name =
            document
                .selectFirst(".main_box09 span")
                ?.text()
                ?.substringBefore("(")
                ?.trim()
                ?: throw BusinessException(StatusCode.SSU5000)
        val values =
            document.select(".main_box09_con li").associate {
                it.selectFirst("dt")?.text().orEmpty() to it.selectFirst("strong")?.text().orEmpty()
            }
        val id =
            values.entries
                .firstOrNull { it.key.contains("학번") }
                ?.value
                ?.toIntOrNull() ?: sIdno
        val rawMajor =
            values.entries
                .firstOrNull { it.key.contains("소속") }
                ?.value
                .orEmpty()
        val status =
            values.entries
                .firstOrNull { it.key.contains("학적") }
                ?.value
                .orEmpty()
