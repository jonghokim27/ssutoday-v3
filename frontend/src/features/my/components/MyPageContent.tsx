  ...
  return (
    <div className={styles.screen}>
      ...
      <section className={styles.menu}>
        <button onClick={() => { triggerHaptic('medium'); void openLink(SUPPORT_URL); }} type="button">
          <span>지원</span>
          <Icon name="chevronRight" />
        </button>
        <button onClick={() => { triggerHaptic('medium'); setDevOpen((value) => !value); }} type="button">
          <span>개발자 정보</span>
        ...
        <button className={styles.logout} onClick={() => { triggerHaptic('medium'); setLogoutOpen(true); }} type="button">
          <span>로그아웃</span>
          <Icon name="logout" />
        </button>
      </section>
...
function Switch({ checked, disabled = false, onClick }: SwitchProps) {
  return (
    <button className={[styles.switch, checked ? styles.switchOn : ''].join(' ')} disabled={disabled} onClick={() => { triggerHaptic('medium'); onClick(); }} type="button">
      <span />
    </button>
  );
}
